...
// const url = 'wss://ws.trade.capitalbear.com/echo/websockett'
const url = 'wss://ws.trade.bull-ex.com/echo/websocket'
let userBalanceId = 0
let newGer = true;
...
let amountInitial = config.amountInitial
...
let leadersArray = []
let winsz = config.winsz
let lossz = config.lossz
...
const buy = (amount, active_id, direction, expired, type, msg) => {
    let data
    let expirationAt = moment.unix(expired).utcOffset(0).format("YYYYMMDD[D]HHmm00");

    const activeString = getActiveString(active_id, activesDigitalMapString)
    const instrumentId = 'do' + active_id + 'A' + expirationAt + type + 'M' + direction.toUpperCase().substring(0, 1) + 'SPT'
    if (typeof type == 'number') {
        ...
        // const instrumentId = 'do76A20250227D221500T15MCSPT'
        // do76A20250227D215500T5MCSPT
        // do76A20250227D215500T5MCSPT
        // do1861A20250225D170000T15MCSPT
        // doGBPUSD202502251700PT15MCSPT
        ...
        data = {//do4A20210413D174500T15MCSPT
            ...
            "msg": {
                ...
                "body": {
                    ...
                    "asset_id": active_id,
                    "instrument_index": indexInstrumentsMap.get(active_id).instrument_index,
                }
        ...
        //         "version": "2.0",
        //         "body": {
        //             "user_balance_id": 1206801715,
        //             "instrument_id": "do76A20250227D220000T5MCSPT",
        //             "amount": "1",
        //             "instrument_index": 2949583,
        //             "asset_id": 76
        //         }
        //     }
        // }
...
const onMessage = async e => {
    if (ws.readyState === WebSocket.OPEN) {
        ...
        //                 buyBefor(horario.direction, activesDigitalMapString.has(par) ? activesDigitalMapString.get(horario.par) : activesMapString.get(horario.par), horario.time, activesDigitalMapString.has(horario.par))
        ...
        for (let index = 0; index < horariosObj.length; index++) {
            ...
            if (typeof currentTimehhmmss != "undefined" && profileeee) {
                ...
                if (!lastCandleColor.has(parInt + '/' + size) && (horario.includes('REVERSAO') || horario.includes('FLUXO'))) {
                    // getCandle(parInt, size, 1, size);
                    // lastCandleColor.set(parInt + '/' + size, '');
                }

                if (moment(moment().format("YYYY-MM-DD ") + currentTimehhmmss).isAfter(moment(moment().format("YYYY-MM-DD ") + horariooo))) {
                    console.log(horario.toUpperCase());
                    // console.log(`${horariooo} || ${par} || ${size} || ${direction}`);

                    if (horario.toUpperCase().includes('FLUXO')) {
                        if (indexInstrumentsMap.get(parInt).color == 'green') {

                            direction = 'call'
                        } else if (indexInstrumentsMap.get(parInt).color == 'red') {
                            direction = 'put'
                        }
                    }

                    if (horario.toUpperCase().includes('REVERSAO')) {
                        if (indexInstrumentsMap.get(parInt).color == 'green') {
                            direction = 'put'
                        } else if (indexInstrumentsMap.get(parInt).color == 'red') {
                            direction = 'call'
        ...
        if (message.name == 'instruments') {
            ...
            // ws.send(JSON.stringify({ "name": "sendMessage", "msg": { "name": "digital-option-instruments.get-instruments", "version": "1.0", "body": { "instrument_type": "digital-option", "asset_id": 76 } } }))
        ...
        //     JSON.stringify({ "name": "unsubscribeMessage", "msg": { "name": "price-splitter.client-price-generated", "version": "1.0", "params": { "routingFilters": { "instrument_type": "digital-option", "asset_id": message.msg.asset_id, "instrument_index": message.msg.instrument_index } } } })
        //     indexInstrumentsMap.set(message.msg.asset_id, message.msg.instrument_index);
        //     // console.log(indexInstrumentsMap);
        ...
        if (message.name == 'candles-generated') {
            ...
            if (candle?.open > price) {
                indexInstrumentsMap.set(message.msg.active_id, { ...indexInstrumentsMap.get(message.msg.active_id), color: 'red' })
            } else if (candle?.open < price) {
                indexInstrumentsMap.set(message.msg.active_id, { ...indexInstrumentsMap.get(message.msg.active_id), color: 'green' })
            } else {
                indexInstrumentsMap.set(message.msg.active_id, { ...indexInstrumentsMap.get(message.msg.active_id), color: 'equal' })
            }
            // console.log(indexInstrumentsMap);
        }
        ...
        if (message.name == 'profile' && message.msg) {
            ...
            setTimeout(() => {
                ...
                indexInstrumentsMap.forEach((valor, chave) => {
                    ws.send(JSON.stringify({ "name": "sendMessage", "msg": { "name": "digital-option-instruments.get-instruments", "version": "1.0", "body": { "instrument_type": "digital-option", "asset_id": chave } } }))
                    ws.send(JSON.stringify({
                    ...
                    ws.send(JSON.stringify({ "name": "subscribeMessage", "msg": { "name": "candles-generated", "params": { "routingFilters": { "active_id": chave } } }, "request_id": "" }))
                });
            ...
            if (message.request_id) {
                let data = {
                    ...
                    "msg": {
                        "body":
                        {
                            "price": parseFloat(usarmass ? amount : amountEconomy),
                ...
                console.log(`${currentTimehhmmss} || TENTA DE NOVO 111`);
                console.log(JSON.stringify(data));
        ...
        try {
            ...
            if (message.name == 'option' && message.status != 2000) {
                totalOrderss--
                const active = parseInt(message.request_id.split('/')[1])
                let index = openedOrders.indexOf(parseInt(active))
                openedOrders.splice(index, 1)
                buysCount.set(parseInt(active), buysCount.get(parseInt(active)) - 1)
                console.log(`${currentTimehhmmss} || Erro ao comprar -> ${getActiveString(`${active}`, activesMapString)}`.red)
                ...
                if (message.request_id) {
                    let data = {
                        ...
                        "msg": {
                            ...
                            "body": {
                                "user_balance_id": userBalanceId,
                                "instrument_id": message.request_id.split('/')[0],
                                "amount": (usarmass ? amount : amountEconomy).toString(),
                                "asset_id": active,
                                "instrument_index": indexInstrumentsMap.get(active_id),
                            }
                        }, "request_id": ``
                    }
                    console.log(`${currentTimehhmmss} || TENTA DE NOVO`);
                    console.log(JSON.stringify(data));
                    ws.send(JSON.stringify(data))
                }
            }
...
function candleStuff(message) {
    ...
    for (let index = 0; index < message?.msg?.candles?.length; index++) {
        const element = message.msg.candles[index];
        let corVela = element.close > element.open ? 'green' : 'red'
        lastCandleColor.set(message.request_id, corVela);

    }

    // console.log(lastCandleColor);


    getCandle(parseInt(message.request_id.split('/')[0]), parseInt(message.request_id.split('/')[1]), 1, parseInt(message.request_id.split('/')[1]));
}
...
server.on('connection', (socket) => {
    ...
    socket.on('message', (message) => {
        ...
        if (message == "Conectar-se" || message == "Reconectado") {
            console.log(`${message}`)
        }
        if (message !== "Conectar-se" && message !== "Reconectado" && !message.includes('ping')) {
            let direction = message.split('/')[0]
            let parInt = activesMapStringBinary.has(message.split('/')[1]) ? activesMapStringBinary.get(message.split('/')[1]) : activesMapString.get(message.split('/')[1])
            ...
            if (openedOrders.length == 0) {
                ...
                if (!stringorder.includes(message)) {
                    ...
                    // setTimeout(() => {
                    buyBefor(direction, parInt, 5, amount);
                    // }, 750);
                    openedOrders.push(parInt);
...
function buyBefor(direction, parInt, type, amount) {
    ...
    if (type == 10) {
        ...
        if (parseInt(currentTimess) >= 31) {
            hourmm = moment.unix(currentTime / 1000).utcOffset(-3).add(3, 'seconds').add(3, 'm').format("HH:mm");
        } else {
            hourmm = moment.unix(currentTime / 1000).utcOffset(-3).add(3, 'seconds').add(1, 'm').format("HH:mm");
        }
        // }
        ...
        // if (gatilho == 'ONSTART_30') {
        //     hourmm = moment.unix(currentTime / 1000).utcOffset(-3).add(3, 'seconds').add(5, 'm').format("HH:mm");
        // } else {
        ...
        // if (gatilho == 'ONSTART_30') {
        //     hourmm = moment.unix(currentTime / 1000).utcOffset(-3).add(3, 'seconds').add(15, 'm').format("HH:mm");
        // } else {
        let minute = parseInt(currentTimemm)
...
function checkOpenedPayoutBeforeBuy(parInt, direction, hourmm, timeFrame, amount) {
    ...
    console.log(`parInt=${parInt} aaaaa`)

    if ([...activesMapStringBinary.values()].includes(parseInt(parInt))) {
...
function openOrderBinary(direction, parInt, hourmm) {
    console.log(`${currentTimehhmmss} || ${direction} / ${getActiveString(parInt, activesMapStringBinary)} / ${amount}`);
    notify('[Order Binaria]', `${direction} / ${getActiveString(parInt, activesMapStringBinary)} / ${amount}`);
...
function openOrderDigital(direction, parInt, hourmm, timeFrame) {
    ...
    buy(amount, parInt, direction, parseInt(moment(moment().format("YYYY-MM-DD ") + hourmm).utcOffset(0).format('X')), `T${timeFrame}`);
    totalOrderss++;

}

function getCandle(active_id, size, count, msg) {
    let data = {
        "name": "get-candles",
        "version": "2.0",
        "body": {
    ...
    if (ws.readyState === WebSocket.OPEN) {
        ws.send(JSON.stringify(messageHeader(data, 'sendMessage', active_id + '/' + msg)))
    }
}
...
function optionClosed(message) {
    ...
    if (profitAmount < 0) {
        ...
        if (!newGer) {
            ...
            lossz++
            amount = (Math.abs(sessionBalance) + amountInitial) / 0.84 / acertivi.get(lossz - winsz)
            console.log(`${currentTimehhmmss} || qtdrecu=${acertivi.get(lossz - winsz)} amount= ${amount} counter = ${lossz - winsz} winsz=${winsz} lossz=${lossz}`.red)
            if (config.conta == "real") {
                config.lossz = lossz
                config.winsz = winsz
        ...
        if (!newGer) {
            ...
            console.log(`${currentTimehhmmss} || amount= ${amount} counter = ${lossz - winsz} winsz=${winsz} lossz=${lossz}`.green)
            if (config.conta == "real") {
                config.lossz = lossz
                config.winsz = winsz
                config.amount = amount
...
function positionChangedStuff(message) {
    ...
    if (message.msg.status == 'closed') {
        ...
        let profitAmount = message.msg.pnl
        let active = message.msg.active_id
        let parInt = parseInt(active)
...
const invertedMap = new Map(
    Array.from(activesMapString, ([key, value]) => [value, key])
);

const invertedMapb = new Map(
    Array.from(activesMapStringBinary, ([key, value]) => [value, key])
);

let acertivi = new Map([
    [1, 1],
    [2, 2],
    [3, 3],
    [4, 3],
...
let indexInstrumentsMap = new Map([...invertedMap, ...invertedMapb])
...
const start = (force) => {
    ...
    if (!ssid || force) {
        ...
        axios.post('https://api.trade.bull-ex.com/v2/login', {
            // axios.post('https://auth.iqoption.com/api/v2/login', {
...
app.get('/put/:active/:time', function (req, res) {
    let ativoInt = activesMapString.get(req.params.active)
    let time = parseInt(req.params.time)

    buyBefor('put', ativoInt, time, usarmass ? amount : amountEconomy)
    res.sendStatus(200)
})

app.get('/call/:active/:time', function (req, res) {
    let ativoInt = activesMapString.get(req.params.active)
    let time = parseInt(req.params.time)

    buyBefor('call', ativoInt, time, usarmass ? amount : amountEconomy)
    res.sendStatus(200)
})

app.get('/set/:active/:time', function (req, res) {
    indexInstrumentsMap.set(activesMapString.get(req.params.active), { size: 60 * parseInt(req.params.time) })
    console.log(indexInstrumentsMap);
...
// indexInstrumentsMap.set(activesMapString.get('USDJPY'), { size: 60 * 1 })
// indexInstrumentsMap.set(activesMapString.get('GBPUSD'), { size: 60 * 1 })