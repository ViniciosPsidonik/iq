...
//             if(parseInt(currentTimehh) >= 6 && parseInt(currentTimehh) <= 14)
...
const onMessage = e => {
    if (ws.readyState === WebSocket.OPEN) {
        ...
        if (stopWinInfinity > 0 && winsInfinity >= stopWinInfinity) {
            notify('Stop', `Stop Win Alcançado...`);
            console.log('Stop Win Alcançado...')
            process.exit(1)
        }
        ...
        if (message.name == 'option-opened' && typeof message.status == "undefined" && parseInt(currentTimehh) >= 6 && parseInt(currentTimehh) <= 14) {
        ...
        if (message.name == 'balance-changed') {
            if (message.msg.current_balance == 1)
                bancaAtual = message.msg.current_balance.amount
        }
...
function optionClosed(message) {
    let amountOption = message.msg.amount
    let active = message.msg.active_id

    if (message.msg.balance_type_id == 1) {
        ...
        if (message.msg.result == 'win') {
            profitAmount = (message.msg.amount) * ((message.msg.profit_percent - 100) / 100)
        } else {
            profitAmount = (message.msg.amount) * -1
        }
        ...
        if (profitAmount < 0) {
            ...
            if (infinity == 14) {
                if (passoSorosLoss == 2) {
                    amount = ((amountLast ? amountLast : amount) * 1.36).toFixed(2)
                } else if (passoSorosLoss < 7) {
                    amount = ((amountLast ? amountLast : amount) * 1.4).toFixed(2)
                } else if (passoSorosLoss == 7) {
                    amount = ((amountLast ? amountLast : amount) * 1.401).toFixed(2)
                } else if (passoSorosLoss == 8) {
                    amount = ((amountLast ? amountLast : amount) * 1.4).toFixed(2)
                } else if (passoSorosLoss == 9) {
                    amount = ((amountLast ? amountLast : amount) * 1.401).toFixed(2)
                } else if (passoSorosLoss == 10) {
                    amount = ((amountLast ? amountLast : amount) * 1.401).toFixed(2)
                } else if (passoSorosLoss == 11) {
                    amount = ((amountLast ? amountLast : amount) * 1.4).toFixed(2)
                } else if (passoSorosLoss == 12) {
                    amount = ((amountLast ? amountLast : amount) * 1.401).toFixed(2)
                } else if (passoSorosLoss == 13) {
                    amount = ((amountLast ? amountLast : amount) * 1.4005).toFixed(2)
                } else if (passoSorosLoss == 14) {
                    amount = ((amountLast ? amountLast : amount) + profitAmount - 10).toFixed(2)
                }
            } else {
                amount = ((amountLast ? amountLast : amount) * 1.4).toFixed(2)
            }

            if (amount > amountSesion + parseFloat(sessionBalance)) {
                console.log(`${currentTimehhmmss} || amount > amountSesion + parseFloat(sessionBalance) | amount = ${amount} amountSesion + parseFloat(sessionBalance) = ${amountSesion + parseFloat(sessionBalance)}`)
                amount = amountSesion + parseFloat(sessionBalance)
            }

            if (bancaAtual < amount) {
                console.log(`${currentTimehhmmss} || || bancaAtual < amount | amount = ${amount} bancaAtual = ${bancaAtual}`)
                amount = bancaAtual
            }

            amountLast = undefined

            console.log(`${currentTimehhmmss} || passoSorosLoss = ${passoSorosLoss}`)
            ...
            if (passoSoros == 2) {
                ...
                if (sorosNoSoros) {
                    if (infinity == 14) {
                        amount = ((parseFloat(amountInitial) + parseFloat(sessionBalance)) * (0.42 / 100)).toFixed(2)
                    } else {
                        amount = ((parseFloat(amountInitial) + parseFloat(sessionBalance)) * (1.4 / 100)).toFixed(2)
                    }
                } else {
                    amount = parseFloat(amounteee)
                }
                amountSesion = parseFloat(amountInitial) + parseFloat(sessionBalance)

                // console.log(amounteee);
                // console.log(sessionBalance);
                // console.log(amountSesion);

                winsInfinity++

                console.log(`${currentTimehhmmss} || new Amount = ${amount} WINS = ${winszao} amountSesion = ${amountSesion} win= ${uin} loss=${los}`.blue)


            } else if (passoSoros == 1) {
                amountLast = amount
                // amount = parseFloat(amount) + parseFloat(profitAmount.toFixed(2))
                amount = parseFloat(amount) + (amount * 0.87)
            }
        }
