    ...
    if (ws.readyState === WebSocket.OPEN) {
        ...
        for (let index = 0; index < horariosObj.length; index++) {
            ...
            if (!indexInstrumentsMap.has(parInt))
                indexInstrumentsMap.set(parInt, index)
            ...
            if (typeof currentTimehhmmss != "undefined" && profileeee) {
                ...
                if (moment(moment().format("YYYY-MM-DD ") + currentTimehhmmss).isAfter(moment(moment().format("YYYY-MM-DD ") + horariooo))) {
                    ...
                    if (horario.toUpperCase().includes('FLUXO')) {
                        if (lastCandleColor.get(parInt + '/' + size) == 'green') {

                            direction = 'call'
                        } else if (lastCandleColor.get(parInt + '/' + size) == 'red') {
                            direction = 'put'
                        }
                    }
                    ...
                    buyBefor(direction, parInt, time, amountEconomy)
                    // indexesss.push(index)
        ...
        if (message.name == 'instruments') {
            ...
            // console.log(mssg);
            // ws.send(mssg)
            // indexInstrumentsMap.set(message.msg.instruments[0].asset_id, message.msg.instruments[0].index);
            // console.log(indexInstrumentsMap);
        }

        if (message.name == 'instrument-generated') {
            indexInstrumentsMap.set(message?.msg.asset_id, message?.msg?.index);
            console.log(indexInstrumentsMap);
            ...
            if (message.request_id) {
                let data = {
                    ...
                    "msg": {
                        "body":
                        {
                            "price": parseFloat(amountEconomy),
...
function checkOpenedPayoutBeforeBuy(parInt, direction, hourmm, timeFrame, amount) {
    ...
    if (activesDigitalMapStringDD.has(getActiveString(parInt, activesMapString)) && false || true) {
