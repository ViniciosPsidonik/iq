...
tail = new Tail('C:/Users/vinícios.psidonik/AppData/Roaming/MetaQuotes/Terminal/CA55B47910D630DFC3635CE516BE3CAD/MQL4/Files/' + moment().format("YYYYMMDD") + "_retorno.csv", "\n", {}, true);
...
tail.on("line", function (data) {
    ...
    if (!openedOrders.includes(parInt)) {
        ...
        if (!stopOrders && !isStopedPar && (!doispraum || doispraum && openedOrders.length == 0)) {
            // setTimeout(() => {
            let time = parseInt(dataSplited[3])
            if (typeof orderopen[parInt] == "undefined") {
                orderopen[parInt] = { amount: amountInitial, direction: dataSplited[2], nogale: false, time }
            } else {
                orderopen[parInt] = { ...orderopen[parInt], direction: dataSplited[2], time }
            }

            if (!orderopen[parInt].nogale) {
                buyBefor(dataSplited[2], parInt, time, amountInitial)
...
const loginFirst = async (page, webSocketMessages) => {
    ...
    const responseexact = await page.waitForResponse(response =>
        response.url() === 'https://api.trade.exnova.com/v2/login' && response.status() === 200
    );
...
const start = (force) => {
    if (!ssid || force) {
        ...
        axios.post('https://api.trade.exnova.com/v2/login', {
            // axios.post('https://auth.iqoption.com/api/v2/login', {
