...
(async () => {
    ...
    //     if (response.url() == "https://api.trade.xoption.com/v2/login") {
    //         console.log({
    //             type: 'response',
    //             url: response.url(),
    ...
    let url
    if (fazlogin) {
        url = 'https://trade.xoption.com/pt/login'
    } else {
        url = 'https://trade.xoption.com/traderoom/'
    }

    await page.goto(url, { timeout: 0, waitUntil: "domcontentloaded", executablePath: 'C:/Program Files/Google/Chrome/Application/chrome.exe' });
...
const loginFirst = async (page, webSocketMessages) => {
    ...
    let text = 'vinipsidonik@gmail.com'; // Substitua pelo texto desejado
...
function buyBefor(direction, parInt, type, gatilho) {
    ...
    if (type == 10) {
        ...
        // } else {
        // if (parseInt(currentTimess) >= 31) {
        hourmm = moment.unix(currentTime / 1000).utcOffset(-3).add(2, 'seconds').add(3, 'm').format("HH:mm");
        // } else {
        //     hourmm = moment.unix(currentTime / 1000).utcOffset(-3).add(2, 'seconds').add(1, 'm').format("HH:mm");
        // }
        // }
