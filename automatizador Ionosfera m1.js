...
tail.on("line", function (data) {
    ...
    if (!achouu && !openedOrders.includes(parInt) && parseInt(currentTimess) <= 05) {
        ...
        if (!stopOrders && !isStopedPar && (!doispraum || doispraum && openedOrders.length == 0)) {
            buyBefor(dataSplited[2], parInt, 1)
        }
    } else {
...
function buyBefor(direction, parInt, type) {
    ...
    let hourmm
    hourmm = moment.unix(currentTime / 1000).utcOffset(-3).add(2, 'seconds').add(timeFrame, 'm').format(" HH:mm");
...
function openOrderDigital(direction, parInt, hourmm, timeFrame) {
    ...
    buy(amount, parInt, direction, parseInt(moment(moment().format("YYYY-MM-DD ") + hourmm).utcOffset(0).format('X')), 'PT' + 1 + 'M');
    totalOrderss++;
    openedOrders.push(parInt);
}
