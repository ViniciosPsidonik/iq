...
let mesmaVela = config.mesmaVela
let paresparaver = config.paresparaver.split(',')
...
let minutoatual1 = null
let currentTimessPlus

setInterval(() => {
    if (typeof currentTime != "undefined")
        paresparaver.map(item => {
            getCandle(activesMapString.get(item), 60, 70, currentTime);
            // ws.send(JSON.stringify({ "name": "subscribeMessage", "msg": { "name": "candles-generated", "params": { "routingFilters": { "active_id": item } } }, "request_id": "" }))
        })
}, 900);
const onMessage = async e => {
    if (ws.readyState === WebSocket.OPEN) {
        ...
        if (message.name == 'option') {
            // console.log('RES = ' + e.data);
            let parInt = parseInt(message.msg.act)
            entranaProxVela[parInt] = { ...entranaProxVela[parInt], taxa: message.msg.value }
            // console.log(entranaProxVela);
        }

        let meta = 70

        if (sessionBalance >= meta) {
            console.log('stop WIN'.green);
        } else if (sessionBalance <= meta * -1) {
            console.log('stop WIN'.red);
        }

        if (message.name == 'candles-generated') {
            if (parseInt(currentTimessPlus) >= 59) {
                // minutoatual1 = currentTimemmPlus
                let price = message.msg.value

                for (const [key, value] of Object.entries(entranaProxVela)) {
                    // console.log(parseInt(message.msg.active_id), parseInt(key));
                    if (parseInt(message.msg.active_id) == parseInt(key)) {
                        // console.log(value.jaentrou);
                        let active = message.msg.active_id
                        if (value.jaentrou) {
                            entranaProxVela[key] = { ...entranaProxVela[key], entra: false, jaentrou: false }
                            console.log(value.taxa, price);

                            if (value.direction == 'call') {
                                if (price < value.taxa) {
                                    let valor = entranaProxVela[key].valor
                                    if (entranaProxVela[key].valor >= 40) {
                                        valor = amountInitial
                                    } else {
                                        valor *= 2
                                        buyBefor(value.direction, parseInt(key), 1, valor);
                                        setTimeout(() => {
                                            entranaProxVela[key] = { ...entranaProxVela[key], entra: false, jaentrou: true, valor }
                                        }, 5000);
                                    }
                            ...
                            if (value.direction == 'put') {
                                if (price > value.taxa) {

                                    let valor = entranaProxVela[key].valor
                                    if (entranaProxVela[key].valor >= 20) {
                                        valor = amountInitial
                                    } else {
                                        valor *= 2
                                        buyBefor(value.direction, parseInt(key), 1, valor);
        ...
        if (message.name == 'profile' && message.msg) {
            ...
            // profileStuf1()
            paresparaver.map(item => {
                console.log(JSON.stringify({ "name": "subscribeMessage", "msg": { "name": "candles-generated", "params": { "routingFilters": { "active_id": activesMapString.get(item) } } }, "request_id": "" }));
                ws.send(JSON.stringify({ "name": "subscribeMessage", "msg": { "name": "candles-generated", "params": { "routingFilters": { "active_id": activesMapString.get(item) } } }, "request_id": "" }))
            })
        ...
        if (message.name == 'heartbeat' || message.name == 'timesync') {
            ...
            currentTimessPlus = moment.unix(currentTime / 1000).utcOffset(-3).add(3, 'seconds').format("ss")
            ...
            // ws.send(JSON.stringify({ "name": "heartbeat", "request_id": "218", "local_time": 30763, "msg": { "userTime": currentTime, "heartbeatTime": currentTime } }))
            ...
            if (!minutoatual) {
                minutoatual = currentTimemm
            } else {
                if (minutoatual != currentTimemm) {
                    minutoatual = currentTimemm


                    for (const [key, value] of Object.entries(entranaProxVela)) {
                        if (value.entra) {
                            if (!openedOrders.includes(parseInt(key))) {
                                // if (openedOrders.length == 0) {
                                buyBefor(value.direction, parseInt(key), 1, amountInitial);
                                entranaProxVela[key] = { ...entranaProxVela[key], entra: false, jaentrou: true }
                                openedOrders.push(parseInt(key))
                            }
                        }
                    }
                }
...
function candleStuff(message) {
    ...
    for (let indexj = 1; indexj <= periodF; indexj++) {


        let hl2Acum = 0
        for (let index = message.msg.candles.length - indexj; index >= message.msg.candles.length - (periodF + (indexj - 1)); index--) {
            const element = message.msg.candles[index];
            let price = element.close
            // console.log(index);
        ...
        let smaFast = hl2Acum / periodF
        // console.log(smaFast)

        smaFastArray[indexj - 1] = smaFast

    }

    let smaSlowArray = []
    for (let indexj = 1; indexj <= periodS; indexj++) {


        let hl2Acum = 0
        for (let index = message.msg.candles.length - indexj; index >= message.msg.candles.length - (periodS + (indexj - 1)); index--) {
            const element = message.msg.candles[index];
            let price = element.close
            // console.log(index);
            let high = element.max
        ...
        let smaSlow = hl2Acum / periodS
        // console.log(smaFast)

        smaSlowArray[indexj - 1] = smaSlow

    }

    let buffer1 = []

    for (let indexj = 0; indexj < periodF; indexj++) {

        buffer1[indexj] = smaFastArray[indexj] - smaSlowArray[indexj]
    }



    let wmaArray = []
    for (let indexj = 1; indexj <= 2; indexj++) {

        let count = 5
        let pwAcum = 0
        let pesosAcum = 0
        for (let index = indexj - 1; index < period + (indexj - 1); index++) {
            // const element = message.msg.candles[index];
            // let price = element.close
            // // console.log(index);
            // let high = element.max
        ...
        let wma = pwAcum / pesosAcum
        // console.log(smaFast)

        wmaArray[indexj - 1] = wma

    }

    // console.log(wmaArray[0])
    // console.log(wmaArray[1])

    let buyCondition = buffer1[0] > wmaArray[0] && wmaArray[1] < wmaArray[1] && !(buffer1[0] < wmaArray[0] && buffer1[1] > wmaArray[1])
    // console.log('buyCondition=', buyCondition);
    let buyCondition1 = buffer1[0] > wmaArray[0] && buffer1[1] < wmaArray[1]
    // console.log('buyCondition1=', buyCondition1);
    let parInt = parseInt(message.request_id)
    if (buyCondition1 || buyCondition) {
        if (mesmaVela) {
            // console.log('mesma vela');
            if (typeof entranaProxVela[parInt] == 'undefined') {
                if (!openedOrders.includes(parInt)) {
                    entranaProxVela[parInt] = { entra: false, valor: amountInitial, direction: 'call', jaentrou: false }
                    // console.log('buy', parInt);
                    buyBefor('call', parInt, 10, amountInitial);
                    openedOrders.push(parInt)
                }
            } else {
                if (!openedOrders.includes(parInt)) {
                    buyBefor('call', parInt, 10, entranaProxVela[parInt].valor);
                    openedOrders.push(parInt)
                }
            }
        } else {
            ...
            if (typeof entranaProxVela[parInt] == 'undefined') {
                entranaProxVela[parInt] = { entra: true, valor: amountInitial, direction: 'call', jaentrou: false }
                // console.log('buy', parInt);
            } else {
                if (entranaProxVela[parInt].entra == false) {
                    entranaProxVela[parInt] = { ...entranaProxVela[parInt], entra: true, direction: 'call' }
                    // console.log('buy', parInt);
                }
            }
        }
        ...
        if (typeof entranaProxVela[parInt] == 'undefined') {
            entranaProxVela[parInt] = { entra: false, valor: amountInitial, direction: 'call', jaentrou: false }
            // console.log('sell', parInt);
        } else if (parseInt(currentTimess) > 3 && parseInt(currentTimess) < 59)
            entranaProxVela[parInt] = { ...entranaProxVela[parInt], entra: false }
        // console.log('sell', parInt);
    }


    let sellCondition = buffer1[0] < wmaArray[0] && buffer1[1] > wmaArray[1] && !(buffer1[0] > wmaArray[0] && buffer1[1] < wmaArray[1])
    // console.log('sellCondition=', sellCondition);
    let sellCondition1 = buffer1[0] < wmaArray[0] && buffer1[1] > wmaArray[1]
    // console.log('sellCondition1=', sellCondition1);

    if (sellCondition1 || sellCondition1) {
        if (mesmaVela) {
            // console.log('mesma vela');
            if (typeof entranaProxVela[parInt] == 'undefined') {
                if (!openedOrders.includes(parInt)) {
                    entranaProxVela[parInt] = { entra: false, valor: amountInitial, direction: 'call', jaentrou: false }
                    // console.log('buy', parInt);
                    buyBefor('put', parInt, 10, amountInitial);
                    openedOrders.push(parInt)
                }
            } else {
                if (!openedOrders.includes(parInt)) {
                    buyBefor('put', parInt, 10, entranaProxVela[parInt].valor);
                    openedOrders.push(parInt)
                }
            }
        } else {
            ...
            if (typeof entranaProxVela[parInt] == 'undefined') {
                entranaProxVela[parInt] = { entra: true, valor: amountInitial, direction: 'put', jaentrou: false }
                // console.log('sell', parInt);
            } else {
                if (entranaProxVela[parInt].entra == false) {
                    entranaProxVela[parInt] = { ...entranaProxVela[parInt], entra: true, direction: 'put' }
                    // console.log('sell', parInt);
                }
            }
        }
        ...
        if (typeof entranaProxVela[parInt] == 'undefined') {
            entranaProxVela[parInt] = { entra: false, valor: amountInitial, direction: 'put', jaentrou: false }
            // console.log('sell', parInt);
        } else if (parseInt(currentTimess) > 3 && parseInt(currentTimess) < 59)
            entranaProxVela[parInt] = { ...entranaProxVela[parInt], entra: false }
        // console.log('sell', parInt);
    ...
    // console.log(entranaProxVela.map(x => x.valor));
...
function optionClosed(message) {
    ...
    let entranaProxVelaObj = entranaProxVela[parseInt(active)]
    if (profitAmount < 0) {
        ...
        // if (entranaProxVelaObj.valor >= 40) {
        //     entranaProxVela[parseInt(active)] = { ...entranaProxVela[parseInt(active)], entra: false, jaentrou: false, valor: amountInitial }
        // } else {
        //     entranaProxVela[parseInt(active)] = { ...entranaProxVela[parseInt(active)], entra: false, jaentrou: false, valor: entranaProxVelaObj.valor * 2 }
        // }

        // console.log(entranaProxVela[parseInt(active)]);
        // lossMass('L');
        ...
        if (entranaProxVelaObj.valor >= 56) {
            entranaProxVela[parseInt(active)] = { ...entranaProxVela[parseInt(active)], entra: false, jaentrou: false, valor: amountInitial }
        }
        // console.log(entranaProxVela[parseInt(active)]);
        if (contarPlacar)
