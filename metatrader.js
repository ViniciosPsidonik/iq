...
server.on('connection', (socket) => {
    ...
    socket.on('message', (message) => {
        ...
        if (message !== "Conectar-se" && message !== "Reconectado" && !message.includes('ping')) {
            ...
            let parInt = activesMapString.has(message.split('/')[1]) ? activesMapString.get(message.split('/')[1]) : activesMapStringBinary.get(message.split('/')[1])
...
function checkOpenedPayoutBeforeBuy(parInt, direction, hourmm, timeFrame, amount) {
    ...
    if (activesMapStringBinary.has(getActiveString(parInt, activesMapStringBinary))) {
