...
setInterval(() => {
    if (taxasObj.size > 0) {
        for (let [key, value] of taxasObj) {
            ...
            getCandle(parInt, 60, 2, key.toString())
        }
    }
}, 1000);
...
setTimeout(() => {
    ...
    for (let index = 0; index < paresArray.length; index++) {
        ...
        getCandle(parInt, 86400, 1, parInt + '/' + par)
    }
}, 6000);
...
const onMessage = e => {
    ...
    if (message.name == 'candles') {
        ...
        if (message.request_id.includes('/')) {
            ...
            let parr = parseInt(message.request_id.split('/')[0])
            let min = candles[0].min
            let max = candles[0].max
            ...
            let priceLastClose = candles[candles.length - 2].close
            let priceLastOpen = candles[candles.length - 2].open

            console.log(priceLastClose);

            let activeInt = parseInt(message.request_id)

            for (let [key, value] of retornouCanal) {
                if (key == parseInt(message.request_id)) {
                    let max = value.max
                    let min = value.min

                    if (!!max && max != 0 && priceLastClose < max) {
                        notify('Voltou', `Retornou ao Canal -> ${getActiveString(activeInt, activesMapString)}`);
                        console.log(`${currentTimehhmmss} || Retornou ao Canal -> ${getActiveString(activeInt, activesMapString)}`)
                        retornouCanal.delete(key)
                        resetTaxa(key, activeInt);
                    } else if (!!min && min != 0 && priceLastClose <= min) {
                        notify('Voltou', `Retornou ao Canal -> ${getActiveString(activeInt, activesMapString)}`);
                        console.log(`${currentTimehhmmss} || Retornou ao Canal -> ${getActiveString(activeInt, activesMapString)}`)
                        retornouCanal.delete(key)
                        resetTaxa(key, activeInt);
            ...
            for (let [key, value] of taxasObj) {
                if (key == parseInt(message.request_id)) {
                    ...
                    if (max != 0 && priceLastClose >= max) {
                        notify('Rompeu Resistencia', `Rompeu Resistencia -> ${getActiveString(activeInt, activesMapString)}`);
                        console.log(`${currentTimehhmmss} || Rompeu Resistencia -> ${getActiveString(activeInt, activesMapString)}`)

                        if (retornouCanal.has(activeInt)) {
                            retornouCanal.set(activeInt, {
                                max,
                                min: retornouCanal.get(activeInt).min
                            })
                        } else {
                            retornouCanal.set(activeInt, { max })
                        }
                    ...
                    } else if (min != 0 && priceLastClose <= min) {
                        notify('Rompeu Suporte', `Rompeu Suporte -> ${getActiveString(activeInt, activesMapString)}`);
                        console.log(`${currentTimehhmmss} || Rompeu Resistencia -> ${getActiveString(activeInt, activesMapString)}`)

                        if (retornouCanal.has(activeInt)) {
                            retornouCanal.set(activeInt, {
                                max: retornouCanal.get(activeInt).max,
                                min
                            })
                        } else {
                            retornouCanal.set(activeInt, { max })
...
function resetTaxa(key, activeInt) {
    if (retornouCanal.has(key)) {
        getCandle(key, 86400, 1, key + '/' + getActiveString(activeInt, activesMapString));
        notify('Resetando...', `Resetando -> ${getActiveString(activeInt, activesMapString)}`);
        console.log(`${currentTimehhmmss} || Resetando -> ${getActiveString(activeInt, activesMapString)}`);
    }
}
...
function getCandle(active_id, size, count, request_id) {
    ...
    if (ws.readyState === WebSocket.OPEN){
        console.log(JSON.stringify(messageHeader(data, 'sendMessage', request_id)));
        ws.send(JSON.stringify(messageHeader(data, 'sendMessage', request_id)));
    }
}
