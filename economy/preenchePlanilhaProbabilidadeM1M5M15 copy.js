...
var workbook = XLSX.readFile('./McCaul, membro do Conselho Fiscal.xlsx');
    ...
    if (ws.readyState === WebSocket.OPEN) {
        ...
        if (profileeee && !rodaJob) {
            ...
            while (continuainteracao) {
                ...
                corVelaa = null
                getCandle(activesMapString.get(sheet), size, 1, timestamp);
                await esperarCondicao()
                console.log('corVelaa', corVelaa);
...
console.log("Esperando a condição ser atendida...");
async function esperarCondicao() {

    while (true) {
        // Simula pegar o dado (substitua isso pelo que você está verificando)
        const dado = verificarSeMudou();

        if (dado) {
            // console.log("Condição atendida! Continuando a execução...");
            break;
        }

        // Aguarda 1 segundo antes de verificar novamente para evitar alto consumo de CPU
        await new Promise(resolve => setTimeout(resolve, 1000));
    }
}

function verificarSeMudou() {
    return !!corVelaa
}
...
function candleStuff(message) {
    ...
    for (let index = 0; index < message?.msg?.candles?.length; index++) {
        ...
        let corVela = element.close > element.open ? 'COMPRA' : element.close < element.open ? 'VENDA' : 'PRETO'
        corVelaa = corVela
    }
