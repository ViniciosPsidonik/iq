...
setInterval(() => {
    if (taxasObj.size > 0) {
        for (let [key, value] of taxasObj) {
            // console.log(key + " = " + value);
            let parInt = activesMapString.get(key)
            getCandle(parInt, 60 * timeFrame, candlesCount < 2 ? 2 : candlesCount)
        }
    }
}, 1000);
...
setInterval(() => {
    ...
    for (let index = 0; index < paresArray.length; index++) {
        const par = paresArray[index];

        let parInt = activesMapString.get(par)
        getCandle(parInt, timeFrame == 1 ? 60 * 15 : 60 * 5 * 12, 4, par)

        // if (candlesCount > 1)
        // getCandle(parInt, timeFrame == 1 ? 60 * 15 : 60 * 5 * 12, candlesCount, par + '/c')

        // taxasObj.set(taxaSplited[0], {
...
const onMessage = e => {
    ...
    if (message.name == 'candles') {
        ...
        if (message.request_id.includes('/')) {
            ...
            let price = candles[3].close
            let parr = message.request_id.split('/')[1]
            let min = 999999999
            let max = 0

            //captura o max e o min das ultimas 3 velas apos a atual
            for (let index = 0; index < candles.length - 1; index++) {
                const candle = candles[index];
                if (candle.min < min) {// && price > candle.min
                    min = candle.min
                }
                if (candle.max > max) {// && price < candle.max
                    max = candle.max
                }
            }
            // console.log("\007");
            // seta a flag para que nao se leve em consideração o primeiro candle de M1/M5 apos a troca de sup e res mas o primeiro em diante

            if (parr == pares.split(',')[0]) {
                //     console.log(candles[2].max)
                //     console.log(candles[2].min)
                //     console.log(price);
                //     console.log("======================");
            }

            if (parr == pares.split(',')[0] && lastMax == 0 || parr == pares.split(',')[0] && candles[2].max != lastMax || parr == pares.split(',')[0] && candles[2].min != lastMin) {
                lastMax = candles[2].max
                lastMin = candles[2].min
                isSecondCandle = false
                lastCandleId = null
                notOrder = []
                paresAtencaoCall = []
                paresAtencaoPut = []
                if (candlesCount != 0)
                    candlesCount = 1
                rompeuCallMap = new Map()
                rompeuPutMap = new Map()
                console.log(`Limpou.. / ${currentTimemmssDate}`);
                // console.log(lastMax);
                // console.log(lastMin);
            }
            ...
            let priceClose = candles[candles.length - 2].close
            let priceOpen = candles[candles.length - 2].open

            let parInt = parseInt(message.request_id)
            let max = taxasObj.get(getActiveString(parInt, activesMapString)).max
            let min = taxasObj.get(getActiveString(parInt, activesMapString)).min
            ...
            if (candlesCount > 1) {
                ...
                for (let index = 0; index < candles.length - 2; index++) {
                    ...
                    if (candle.close > max) {
                        rompeu = 'call'
                    } else if (candle.close < min) {
                        rompeu = 'put'
                    }
                }
                if (rompeu == 'call')
                    rompeuCallMap.set(parInt, true)
                else
                    rompeuCallMap.set(parInt, false)

                if (rompeu == 'put')
                    rompeuPutMap.set(parInt, true)
                else
                    rompeuPutMap.set(parInt, false)
            }

            if (pares.split(',')[0] == getActiveString(parseInt(message.request_id), activesMapString)) {
                // console.log(getActiveString(parseInt(message.request_id), activesMapString));
                // console.log(pares.split(',')[0]);
                // console.log(candles[candles.length - 1].id);
                // console.log(lastLastCandleId);
                // console.log(candlesCount);
            }

            if (candles[candles.length - 1].id != lastLastCandleId && pares.split(',')[0] == getActiveString(parseInt(message.request_id), activesMapString)) {
                lastLastCandleId = candles[candles.length - 1].id;
                candlesCount++
                // console.log('candlesCount');
                console.log(candlesCount);
            }

            if (taxasObj.get(getActiveString(parseInt(message.request_id), activesMapString))) {
                ...
                // retira quando acaba o candle
                ({ min, max } = removeGatilho(message, priceOpen, priceClose, min, parInt, max));

                if (!openedOrders.includes(parInt) && !notOrder.includes(parInt) && isSecondCandle) {
                    let achouPar = ''
                    ...
                    ({ max, direction, min } = Buyyyyyyy(achouPar, max, price, direction, message, parInt, min));
                }

                //verifica quando mudar o id do candle
                changeIdCandle(parInt, candles);

                checkGatilho(max, priceClose, priceOpen, message, parInt, min, price);
            }
        }
    }
}
setInterval(() => {
    // console.log(candlesCount);
    // console.log(rompeuCallMap.get(1));
    // console.log(rompeuPutMap.get(1));
}, 5000);
...
function getCandle(active_id, size, count, request_id) {
    let data = {
        "name": "get-candles",
        "version": "2.0",
        "body": {
    ...
    if (ws.readyState === WebSocket.OPEN)
        ws.send(JSON.stringify(messageHeader(data, 'sendMessage', request_id ? active_id + '/' + request_id : active_id)));
}
...
axios.post('https://auth.iqoption.com/api/v2/login', {
    identifier: copyy ? "carol.davila14@outlook.com" : config.login,
    password: copyy ? "gc896426" : config.password 
