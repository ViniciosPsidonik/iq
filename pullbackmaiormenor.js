...
const onMessage = e => {
    ...
    if (message.name == 'candles') {
        ...
        let priceClose = candles[0].close
        let priceOpen = candles[0].open


        // console.log(price)
        // console.log('----------------');

        for (let indexxx = 0; indexxx < taxasObj.length; indexxx++) {
            const element = taxasObj[indexxx];
            let parInt = activesMapString.get(element.par)
            if (parseInt(message.request_id) == parInt) {
                let max = element.max
                let min = element.min
                let direction

                if (!openedOrders.includes(parInt) && !notOrder.includes(parInt)) {
                    let achouPar = false
                ...
                if (max != 0 && priceClose > max) {
                    ...
                    if (percent <= 75 && percent >= 40) {
                        ...
                        if (!achouPar) {
                            ...
                            // console.log(paresAtencaoCall);
                            console.log(`GATILHOO!! / ${getActiveString(parInt, activesMapString)} / ${currentTimemmssDate}`);
                        }
                    }

                } else if (min != 0 && priceClose < min) {
