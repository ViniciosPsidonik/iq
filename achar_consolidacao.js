...
setInterval(() => {
    for (var [key, value] of openedMap) {
        if (value)
            getCandle(key, 60 * 5)
    }
}, 1000)
...
const onMessage = e => {
    ...
    if (message.name == 'heartbeat' || message.name == 'timesync') {
        ...
        if (parseInt(timeIntLast) == 05 && parseInt(currentTimemmss) > 3 && !printou || parseInt(timeIntLast) == 00 && parseInt(currentTimemmss) > 3 && !printou) {
            printou = true
        ...
        } else if (parseInt(timeIntLast) != 05 && parseInt(timeIntLast) != 00) {
            printou = false
        }
        // if (log)
    ...
    if (message.name == 'candles') {
        ...
        for (let index = 0; index < candles.length - 1; index++) {
            ...
            let sizeCandleMax = candle.max - candle.min
            let sizeCandle
            let porcentagemRetraiu = 0
        ...
        porcentagensMap.set(getActiveString(parseInt(message.request_id.split('/')[0]), activesMapString), parseFloat((100 - (porcentagemRetraiuMedia / (candles.length - 1))).toFixed(2)))
    }
}
