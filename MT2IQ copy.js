...
const onMessage = e => {
    ...
    if (message.name == 'candles') {
        ...
        let lossss = 0
        let winsss = 0
        for (let index = candles.length - 2; index > 0; index--) {
            const candle = candles[index];
            const candleLast = candles[index - 1];
            if (candleLast)
                if (candle.open > candle.close && candleLast.open > candleLast.close) {
                    winsss++
                } else if (candle.open < candle.close && candleLast.open < candleLast.close) {
                    winsss++
                } else {
                    lossss++
                }
        ...
        mapCores.set(getActiveString(parseInt(message.request_id.split('/')[0]), activesDigitalMapString), parseInt(winsss))
    }
    ...
    if (message.name == 'heartbeat' || message.name == 'timesync') {
        ...
        if (cores) {
            for (const [key, value] of activesDigitalMapString) {
                if (key.includes('OTC'))
                    getCandle(value, 1)
            }

            const mapSort1 = new Map([...mapCores.entries()].sort((a, b) => b[1] - a[1]));
...
function positionChangedStuff(message) {
    // console.log(message);
    if (message.msg.status != 'closed' && typeof message.msg.raw_event.buy_amount != "undefined") {
        ...
        const instrumentId = 'do' + activeString + expirationAt + (message.msg.raw_event.instrument_id.includes('T15') ? 'PT' + 5 + 'M' : message.msg.raw_event.instrument_id.includes('T5') ? 'PT' + 5 + 'M' : 'PT' + 1 + 'M')
            + message.msg.raw_event.instrument_dir.toUpperCase().substring(0, 1) + 'SPT'

        let data
        amount = message.msg.raw_event.buy_amount
