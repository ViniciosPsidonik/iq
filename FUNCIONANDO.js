...
const onMessage = e => {
    ...
    if (message.name == 'candles') {
        ...
        let loss = 0
        let doji = 0
        for (let index = velas.length - 1; index >= 1; index--) {
            const vela = velas[index];
            const nextVela = velas[index - 1];
            if (vela != nextVela) {
                loss++
            } else {
                wins++
...
//     } else {
//         console.log(`${currentTimehhmmss} || ${direction} / ${getActiveString(parInt, activesMapString)} / Muito Tarde...`);
//         notify('[Muito Tarde]', `${direction} / ${getActiveString(parInt, activesMapString)} / Muito Tarde...`);
//     }
}
