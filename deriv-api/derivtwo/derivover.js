...
var workbook = XLSX.readFile('./Massanielloderiv10050over.xlsx');
var worksheet = workbook.Sheets['Calculadora']
let amount = 0.35
let fsConfig = fs.readFileSync('derivover.json')
let config = JSON.parse(fsConfig)

let fsConfig2 = fs.readFileSync('derivunder.json')
let config2 = JSON.parse(fsConfig2)
...
async function buyRec() {
    ...
    balance = await api.account('0twjsyncz8HSkwL');
...
const esperaPoderEntrar = (type) => {
    return new Promise((resolve, reject) => {
        axios.get(`http://localhost:9091/cantrade`).then((res) => {
            console.log('enviado');
        }).catch((err) => {
...
function save() {
    ...
    fs.writeFile('derivover.json', JSON.stringify(config, null, 4), err => {
    });
}
