...
let horariosJowJow = config.horariosJowJow
...
//             // setTimeout(() => {
//                 if(parseInt(currentTimess) < 10){
//                     buyBefor(dataSplited[2], parInt, dataSplited[3])
//                 } else {
//                     console.log(`${currentTimehhmmss} || parseInt(currentTimess) < 10`);
//                 }
//             // }, 500);
//         }
//     } else {
...
let entradas = horariosJowJow.split(';')
...
const onMessage = e => {
    ...
    if (message.name == 'heartbeat' || message.name == 'timesync') {
        ...
        for (let index = 0; index < entradas.length; index++) {
            ...
            const direcao = entrada.split(' ')[2]
            const moeda = entrada.split(' ')[1]
            const tempo = entrada.split(' ')[0]

            console.log(tempo);
            console.log(currentTimehhmm);
            console.log('=========');

            if(currentTimehhmm == tempo){
                buyBefor(direcao.toLowerCase(), activesMapString.get(moeda), 5)
...
function buyBefor(direction, parInt, type) {
    ...
    checkOpenedPayoutBeforeBuy(parInt, direction, hourmm, timeFrame, type);
}

let notOrder = []

function checkOpenedPayoutBeforeBuy(parInt, direction, hourmm, timeFrame, type) {
    ...
    if(type == 15){
        openOrderDigital(direction, parInt, hourmm, timeFrame, type);
