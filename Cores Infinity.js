...
const onMessage = e => {
    if (ws.readyState === WebSocket.OPEN) {
        ...
        if (message.name == 'candles') {
            ...
            if (currentTimess < 30 && idcandle != candles[1].id) {
                if (candles[0].open > candles[0].close && !positionOpenedSoros && (candles[1].open <= candles[1].close)) {
                    idcandle = candles[1].id
                    buyBefor('put', activeeee, 1)
                } else if (candles[0].open < candles[0].close && !positionOpenedSoros && (candles[1].open >= candles[1].close)) {
                    idcandle = candles[1].id
                    buyBefor('call', activeeee, 1)
                }

            } 
            // else if (minute != currentTimemm && idcandle == candles[1].id) {
            //     if (candles[1].open > candles[1].close && !positionOpenedSoros) {
            //         buyBefor('put', activeeee, 1)
            //     } else if (candles[1].open < candles[1].close && !positionOpenedSoros) {
            //         buyBefor('call', activeeee, 1)
            //     }
            // }
        }
        ...
        if (message.msg.status == 'closed') {
            ...
            if (profitAmount < 0) {
                ...
                if (passoSoros == 2) {
                    ...
                    console.log(`${currentTimehhmmss} || new Amount = ${amount} WINS = ${winsInfinity} amountSesion = ${amountSesion} win= ${uin} loss=${los}`.blue)
