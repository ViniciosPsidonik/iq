...
tail.on("line", function (data) {
    ...
    if (!achouu && !openedOrders.includes(parInt)) {
        checkCandle.push({
            ...
            type: parseInt(dataSplited[3]),
            timeInt: parseInt(currentTimehhmm.substring(currentTimehhmm.length - 2, currentTimehhmm.length)),
        })
...
function buyBefor(direction, parInt, type) {
    ...
    let hourmm
    hourmm = moment.unix(currentTime / 1000).utcOffset(-3).add(2, 'seconds').add(timeFrameL ? timeFrameL : timeFrame, 'm').format(" HH:mm");
