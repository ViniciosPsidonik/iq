...
tail.on("line", function (data) {
    ...
    if (!openedOrders.includes(parInt) && parseInt(currentTimess) <= 6 && (timeInt == 0 || timeInt == 5)) {
        ...
        if (message.request_id.includes('/')) {
            ...
            for (let index = 0; index < checkCandle.length; index++) {
                ...
                if (parseInt(message.request_id.split('/')[0]) == element.parInt) {
                    if (typeof element.candleId == 'undefined') {
                        checkCandle[index] = {
                            candleId: candles[1].id,
                            ...element
                        }
                    } else {
                        if (candles[1].id != element.candleId) {
                            checkCandle.splice(index, 1)
                            index--
                            continue
                        }
                    }
                    if (element.direction == 'call' && price <= open) {
                        buyBefor(element.direction, element.parInt, 5)
                        checkCandle.splice(index, 1)
                        index--
                    } if (element.direction == 'put' && price >= open) {
                        buyBefor(element.direction, element.parInt, 5)
                        checkCandle.splice(index, 1)
                        index--
                    }
...
function buyBefor(direction, parInt, type) {
    ...
    let hourmm
    hourmm = moment.unix(currentTime / 1000).utcOffset(-3).add(2, 'seconds').add(timeFrameL ? timeFrameL : 5, 'm').format(" HH:mm");
