...
tail.on("line", function (data) {
    ...
    if (!achouu && !openedOrders.includes(parInt)) {
        ...
        if (!stopOrders && !isStopedPar && (!doispraum || doispraum && openedOrders.length == 0)) {
            ...
            // futureBuys.push({
            //     direction: dataSplited[2],
            //     parInt,
            //     time: dataSplited[3],
            //     minuteToSend: currentTimemm
            // })

            console.log(futureBuys);
        }
    } else {
...
const onMessage = e => {
    ...
    if (message.name != 'instrument-quotes-generated' && message.name != 'api_option_init_all_result')
        console.log('RES = ' + e.data)
// 
    for (let index = 0; index < futureBuys.length; index++) {
        const element = futureBuys[index];
        if (parseInt(currentTimemm) == parseInt(element.minuteToSend) + 1 || element.minuteToSend == '59' && parseInt(currentTimemm) == 0) {
            let isStopedPar = false
            for (let index = 0; index < stopOrdersPares.length; index++) {
                const stopOrdersPar = stopOrdersPares[index];
                if (getActiveString(element.parInt, activesMapString).includes(stopOrdersPar)) {
            ...
            if (!stopOrders && !isStopedPar && (!doispraum || doispraum && openedOrders.length == 0)) {
                buyBefor(element.direction, element.parInt, element.time)
                futureBuys.splice(0, 1)
            }
        }
    ...
    if (message.name == 'option-opened' && message.status != 0) {
        console.log(message.msg.expiration_time);
        console.log(parseInt(moment(moment().format("YYYY-MM-DD ") + moment.unix(message.msg.expiration_time).format("hh:mm")).utcOffset(0).format('X')))

        parseInt(moment(moment().format("YYYY-MM-DD ") + moment.unix(message.msg.expiration_time).format("hh:mm")).utcOffset(0).format('X'))
        let data = {
            "name": "sendMessage",
            "msg": {
                "body":
                {
                    "price": parseFloat(amount),
                    "active_id": 76,
                    "expired": parseInt(message.msg.expiration_time),
                    "direction": message.msg.direction,
                    "option_type_id": 1,//turbo 1 binary
                    "user_balance_id": userBalanceId
                }
                , "name": "binary-options.open-option", "version": "1.0"
            },
...
function buyBefor(direction, parInt, type) {
    ...
    let hourmm
    hourmm = moment.unix(currentTime / 1000).utcOffset(-3).add(2, 'seconds').add(type == 1 ? type : timeFrameL, 'm').format(" HH:mm");
...
function optionClosed(message) {
    ...
    if (profitAmount < 0) {
        ...
        losss++
        console.log(`${currentTimehhmmss} || ${profitAmount < 0 ? "Loss" : "Win"} ${profitAmount.toFixed(2)} / Balance: ${parseFloat(sessionBalance.toFixed(2))} / ${getActiveString(active, activesMapString) ? getActiveString(active, activesMapString) : active} / Binario / losss=${losss} || winss=${winss}`.red)
        ...
        console.log(`${currentTimehhmmss} || ${profitAmount < 0 ? "Loss" : "Win"} ${profitAmount.toFixed(2)} / Balance: ${parseFloat(sessionBalance.toFixed(2))} / ${getActiveString(active, activesMapString) ? getActiveString(active, activesMapString) : active} / Binario / losss=${losss} || winss=${winss}`.green)
