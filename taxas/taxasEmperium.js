            ...
            if (taxasEmper.par == message.msg.active_id) {
                ...
                if (taxasEmper.direction == 'call') {
                    if (close <= taxasEmper.taxa) {
                        console.log('taxasEmper.countid=', taxasEmper.countid);
                        buyBefor('call', message.msg.active_id, taxasEmper.time);
                        openedOrderIds.push(taxasEmper.countid)
                    }
                } else {
                    if (close >= taxasEmper.taxa) {
                        console.log('taxasEmper.countid=', taxasEmper.countid);
                        buyBefor('put', message.msg.active_id, taxasEmper.time);
                        openedOrderIds.push(taxasEmper.countid)
                    }
                }
            }
...
function buyBefor(direction, parInt, type) {
    ...
    if (type == 1) {
        ...
        if (parseInt(currentTimess) >= 30 && (minute == 14 || minute == 29 || minute == 44 || minute == 59)) {
            hourmm = moment.unix(currentTime / 1000).utcOffset(-3).add(2, 'seconds').add(6, 'm').format("HH:mm");
            return
        } else {
            hourmm = moment.unix(currentTime / 1000).utcOffset(-3).add(2, 'seconds').add(timeFrameL, 'm').format("HH:mm");
...
for (let index = 0; index < textArray.length; index++) {
    const element = textArray[index].split(' ');
